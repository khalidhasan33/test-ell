...
package com.choosemuse.example.unity;
...
public class LibmuseUnityAndroid {
    /**
    ...
    private final Accelerometer[] accelMap =
            new Accelerometer[]{Accelerometer.X, Accelerometer.Y, Accelerometer.Z};
    ...
    private final Gyro[] gyroMap = new Gyro[]{Gyro.X, Gyro.Y, Gyro.Z};
    private final Eeg[] eegMap = new Eeg[]{Eeg.EEG1, Eeg.EEG2, Eeg.EEG3, Eeg.EEG4, Eeg.AUX_LEFT, Eeg.AUX_RIGHT};
    private final Ppg[] ppgMap = new Ppg[]{Ppg.AMBIENT, Ppg.IR, Ppg.RED};

    // Debug string
    private final String TAG = "LibmuseUnityAndroid";

    //--------------------------------------
    // Constructor and public method that can be called from C#

    public LibmuseUnityAndroid(Activity context) {
        if(context == null) {
    ...
    public void connect(String headband) {
        ...
        Muse museObj = museHashMap.get(headband);
        if (museObj != null) {
            manager.stopListening();
            muse = museObj;
            ...
            // Choose which signals Muse should send
    ...
    private void museListChanged() {
        ...
        StringBuilder muses = new StringBuilder();
        for(Muse m : list) {
            museHashMap.put(m.getName(), m);
            muses.append(m.getName()).append(" ");
        }
        ...
        for(ArrayList<String> listener: unityMuseListener) {
            UnityPlayer.UnitySendMessage(listener.get(0), listener.get(1), muses.toString());
        }
    }
    ...
    private void initDataStructures() {
        unityDataListener = new ArrayList<>();
        unityArtifactListener = new ArrayList<>();
        unityConnectionListener = new ArrayList<>();
        unityMuseListener = new ArrayList<>();
        museHashMap = new HashMap<>();
        dataTypeToListen = new ArrayList<>();
        stringToDataPacketType = new HashMap<>();
        dataPacketTypeToString = new HashMap<>();
        initMaps();
    }
    ...
    private void initMaps() {
        ...
        stringToDataPacketType.put("PPG", MuseDataPacketType.PPG);
    ...
    private JSONArray createDataJSONArray(final MuseDataPacket p) {
        ...
        try{
            switch (p.packetType()) {
                ...
                case PPG:
                    for(int i = 0; i < p.valuesSize(); i++) {
                        result.put(p.getPpgChannelValue(ppgMap[i]));
                    }
                    break;
    ...
    // outer class. Each class simply forwards the messages it receives back to the LibmuseUnityAndroid.
    class MuseL extends MuseListener {

        @Override
        public void museListChanged() {
            LibmuseUnityAndroid.this.museListChanged();
        }
    }
    ...
    class ConnectionListener extends MuseConnectionListener {
        ...
        public void receiveMuseConnectionPacket(final MuseConnectionPacket p, final Muse muse) {
            LibmuseUnityAndroid.this.receiveMuseConnectionPacket(p, muse);
        }
    }
    ...
    class DataListener extends MuseDataListener {
        ...
        public void receiveMuseDataPacket(final MuseDataPacket p, final Muse muse) {
            LibmuseUnityAndroid.this.receiveMuseDataPacket(p, muse);
        }

        @Override
        public void receiveMuseArtifactPacket(final MuseArtifactPacket p, final Muse muse) {
            LibmuseUnityAndroid.this.receiveMuseArtifactPacket(p, muse);
        }
    }
}
