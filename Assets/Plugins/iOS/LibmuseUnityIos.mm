...
+(LibmuseUnityIos *) getInstance {
    @synchronized(self) {
        if (!instance)
            (void)[[LibmuseUnityIos alloc] init];
        return instance;
    }
    return nil;
}
...
- (void) connect: (NSString*) headband {
    ...
    if (muse != nil) {
        ...
        // Set preset with PPG
        [self.muse setPreset:IXNMusePresetPreset51];
...
- (void) registerDataPacketTypes {
    for(NSString * type in self.dataPacketToListen) {
        if ([type isEqualToString:@"ACCELEROMETER"]) {
            ...
            [self.muse registerDataListener:self type:IXNMuseDataPacketTypePpg];
        } else if ([type isEqualToString:@"TEMPERATURE"]) {
            [self.muse registerDataListener:self type:IXNMuseDataPacketTypeTemperature];
...
- (NSString*) createStateString:(IXNConnectionState) status {
    ...
    switch (status) {
        ...
        case IXNConnectionStateNeedsUpdate:
            state = @"NEEDS_UPDATE";
            break;
        case IXNConnectionStateNeedsLicense:
            state = @"NEEDS_LICENSE";
            break;
...
- (NSString*) createPacketString:(IXNMuseDataPacketType) type {
    ...
    switch (type) {
        ...
        case IXNMuseDataPacketTypeEeg:
            packet = @"EEG";
            break;
        case IXNMuseDataPacketTypePpg:
            packet = @"PPG";
            break;
        case IXNMuseDataPacketTypeTemperature:
            packet = @"TEMPERATURE";
            break;
...
- (NSString*) createDataString:(IXNMuseDataPacket*) packet {
    ...
    switch (packet.packetType) {
        case IXNMuseDataPacketTypeAccelerometer:
            data = [NSString stringWithFormat:@"[%.2f, %.2f, %.2f]",
                    [packet getAccelerometerValue:IXNAccelerometerX],
                    [packet getAccelerometerValue:IXNAccelerometerY],
                    [packet getAccelerometerValue:IXNAccelerometerZ]];
            break;
        case IXNMuseDataPacketTypeBattery:
            data = [NSString stringWithFormat:@"[%.2f, %.2f, %.2f]",
            ...
            break;
        case IXNMuseDataPacketTypeDrlRef:
            data = [NSString stringWithFormat:@"[%.2f, %.2f]",
                    [packet getDrlRefValue:IXNDrlRefDrl],
                    [packet getDrlRefValue:IXNDrlRefRef]];
            break;
        case IXNMuseDataPacketTypeGyro:
            data = [NSString stringWithFormat:@"[%.2f, %.2f, %.2f]",
                    [packet getGyroValue:IXNGyroX],
                    [packet getGyroValue:IXNGyroY],
                    [packet getGyroValue:IXNGyroZ]];
            break;
        case IXNMuseDataPacketTypePpg:
            data = [NSString stringWithFormat:@"[%.2f, %.2f, %.2f]",
                    [packet getPpgChannelValue:IXNPpgAMBIENT],
                    [packet getPpgChannelValue:IXNPpgIR],
                    [packet getPpgChannelValue:IXNPpgRED]];
            break;
        case IXNMuseDataPacketTypeTemperature:
            data = [NSString stringWithFormat:@"[%.2f]",
                    [packet getTemperatureValue]];
            break;
        case IXNMuseDataPacketTypeEeg:
        case IXNMuseDataPacketTypeAlphaAbsolute:
        case IXNMuseDataPacketTypeBetaAbsolute:
        ...
        case IXNMuseDataPacketTypeHsiPrecision:
        case IXNMuseDataPacketTypeArtifacts:
            data = [NSString stringWithFormat:@"[%.2f, %.2f, %.2f, %.2f, %.2f, %.2f]",
            ...
            break;
        default:
            data = [NSString stringWithFormat:@"[%s]", "Unhandled"];
            break;
    }
    return data;
}
